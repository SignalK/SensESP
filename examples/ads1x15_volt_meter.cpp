...
ReactESP app([] () {
  ...
  // A small arbitrary delay is required to let the serial port catch up
  ...
  // - SKOutputNumber() is a specialized transport to send a float value to the Signal K server.
    ...
    p12V_AltVoltage->connectTo(new ADS1x15Voltage(chip, gain, "/12V_Alt/ADS1x15Voltage")) 
  ...
  auto* p24VAltVoltage = new ADS1115channel(ads1115, channel_24V, read_delay_24V, "/24V_Alt/ADC read delay");
    ...
    p24VAltVoltage->connectTo(new ADS1x15Voltage(chip, gain, "/24V_Alt/ADS1x15Voltage")) 
