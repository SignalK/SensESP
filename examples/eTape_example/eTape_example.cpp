...
class ETapeInterpreter : public CurveInterpolator {
    ...
    public:
        ETapeInterpreter(String config_path="") :
          ...
          // you can enable the output of the eTape sensor resistances to obtain
...
// Some initialization boilerplate when in debug mode...
  ...
  //
  // This example prints out on the IP_ADDRESS from the standard sensors.

  sensesp_app = builder.set_hostname("sensor_name")  // enter a unique name for the device
                ->set_standard_sensors(IP_ADDRESS)
                ->set_sk_server("012.234.567.890", 1111) // insert the SignalK server ip_address and port here
                ->get_app();
...
// the MovingAverage transport before outputting the result. By changing the data in the curve interpolator
// you can print out the %Full dependent on you system geometry.
