...
ReactESP app([] () {
  ...
  const float R1 = 51.0; // The resistance, in ohms, of the fixed resistor (R1) in the voltage divider circuit
  ...
  pAnalogInput->connectTo(new AnalogVoltage()) ->  // convert the value from the AnalogIn pin into a voltage
                connectTo(new VoltageDividerR2(R1, Vin, "/gen/temp/sender")) ->  // convert voltage into ohms
                connectTo(new TemperatureInterpreter("/gen/temp/curve")) ->  // find the Kelvin value for the given ohms value
                connectTo(new Linear(1.0, 0.0, "/gen/temp/calibrate")) ->  // in case you want to calibrate the output at runtime
                connectTo(new SKOutputNumber(sk_path, "/gen/temp/sk"));  // send calibrated Kelvin value to SignalK
