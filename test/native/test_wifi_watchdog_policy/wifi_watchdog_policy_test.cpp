...
void test_not_armed_never_restarts() {
...
void test_armed_down_below_timeout_no_restart() {
  WiFiWatchdogPolicy policy;
...
void test_armed_down_at_timeout_restarts() {
  WiFiWatchdogPolicy policy;
  policy.set_link_state(WiFiWatchdogLink::kUp, 1000);
  policy.set_link_state(WiFiWatchdogLink::kDown, 5000);
...
void test_armed_up_never_restarts() {
  WiFiWatchdogPolicy policy;
  policy.set_link_state(WiFiWatchdogLink::kUp, 1000);
...
void test_reconnect_clears_timer() {
  WiFiWatchdogPolicy policy;
  policy.set_link_state(WiFiWatchdogLink::kUp, 1000);
  policy.set_link_state(WiFiWatchdogLink::kDown, 5000);
...
void test_repeated_down_keeps_original_start() {
  WiFiWatchdogPolicy policy;
  policy.set_link_state(WiFiWatchdogLink::kUp, 1000);
  policy.set_link_state(WiFiWatchdogLink::kDown, 5000);
...
void test_neutral_does_not_arm() {
  WiFiWatchdogPolicy policy;
...
void test_neutral_clears_running_timer() {
  WiFiWatchdogPolicy policy;
  policy.set_link_state(WiFiWatchdogLink::kUp, 1000);
  policy.set_link_state(WiFiWatchdogLink::kDown, 5000);
...
// Neutral clears the outage timer but keeps the policy armed.
void test_armed_survives_neutral() {
  WiFiWatchdogPolicy policy;
  policy.set_link_state(WiFiWatchdogLink::kUp, 1000);
  policy.set_link_state(WiFiWatchdogLink::kDown, 5000);
  policy.set_link_state(WiFiWatchdogLink::kNeutral, 6000);
  policy.set_link_state(WiFiWatchdogLink::kDown, 7000);
  TEST_ASSERT_FALSE(policy.restart_due(7000 + kTimeoutMs - 1, kTimeoutMs));
  TEST_ASSERT_TRUE(policy.restart_due(7000 + kTimeoutMs, kTimeoutMs));
}
...
void test_clock_wraparound() {
  ...
  // Still before the wrap: a non-modular compare would fire here.
  TEST_ASSERT_FALSE(policy.restart_due(down_at + 500, kTimeoutMs));
  TEST_ASSERT_FALSE(policy.restart_due(UINT32_MAX, kTimeoutMs));
  TEST_ASSERT_FALSE(policy.restart_due(due_at - 1, kTimeoutMs));
  TEST_ASSERT_TRUE(policy.restart_due(due_at, kTimeoutMs));
}

void test_zero_timeout_never_restarts() {
  WiFiWatchdogPolicy policy;
  policy.set_link_state(WiFiWatchdogLink::kUp, 1000);
  policy.set_link_state(WiFiWatchdogLink::kDown, 5000);
...
void test_ap_sta_boot_without_station_never_restarts() {
  WiFiWatchdogPolicy policy;
...
int main(int argc, char** argv) {
  ...
  RUN_TEST(test_armed_survives_neutral);
