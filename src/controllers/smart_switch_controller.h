 ...
 * control a LoadController. This controller accepts inputs from a generic
 * boolean producer (usually a SignalK listener), as well as String and
 * ClickType transforms. The latter allows a physical button to control the load
 * as well as add special behaviors to the sensor application. In particular, an
 * ultra long press of the button will cause the MCU to reboot. <p>A
 * SmartSwitchController object behaves differently depending on the type of
 ...
 * type" will be treated as a "click", and will toggle the output btween "on"
 * and "off".
 * @see LoadController
 * @see TextToTruth
 * @see ClickType
