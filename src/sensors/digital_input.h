...
/**
 * @brief DigitalInputState reports the state of a digital pin every read_delay
 * ms.
 *
 ...
 * @param pin_mode Will be INPUT or INPUT_PULLUP.
 *
 * @param interrupt_type Will be RISING, FALLING, or CHANGE, but it doesn't
 * matter which one you use, as this Class doesn't use an interrupt.
 *
...
/**
 * @brief DigitalInputChange is meant to report when a digital pin changes its
 * state.
 *
 * It's intended for monitoring a button or other type of relatively slow
 * changing switch. Because physical switches like buttons can be somewhat
 * "noisy", you may want to connect the output of this Sensor to the Debounce
 * Transform, and try different combinations of read_delay for this Sensor and
 * ms_min_delay for Debounce to get the "cleanest" output from your button or
 * other switch. (Make Debounce::ms_min_delay at least a little longer than
