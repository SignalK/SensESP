...
INA219value::INA219value(INA219* pINA219, INA219ValType val_type,
...
void INA219value::enable() {
  app.onRepeat(read_delay, [this]() {
    switch (val_type) {
      case bus_voltage:
        output = pINA219->pAdafruitINA219->getBusVoltage_V();
        break;
      case shunt_voltage:
        output = (pINA219->pAdafruitINA219->getShuntVoltage_mV() /
                  1000);  // SK wants volts, not mV
        break;
      case current:
        output = (pINA219->pAdafruitINA219->getCurrent_mA() /
                  1000);  // SK wants amps, not mA
        break;
      case power:
        output = (pINA219->pAdafruitINA219->getPower_mW() /
                  1000);  // SK want watts, not mW
        break;
      case load_voltage:
        output = (pINA219->pAdafruitINA219->getBusVoltage_V() +
                  (pINA219->pAdafruitINA219->getShuntVoltage_mV() / 1000));
        break;
      default:
