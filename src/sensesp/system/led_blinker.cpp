...
BaseBlinker::BaseBlinker(int pin) : pin_{pin} {
  pinMode(pin, OUTPUT);
  reactesp::EventLoop::app->onDelay(1, [this]() { this->tick(); });
}
...
void EvenBlinker::tick() {
  ...
  this->flip_state();
  event_ =
      reactesp::EventLoop::app->onDelay(period_, [this]() { this->tick(); });
}
...
void RatioBlinker::tick() {
  ...
  event_ = reactesp::EventLoop::app->onDelay(ref_duration,
                                               [this]() { this->tick(); });
}
...
void PatternBlinker::tick() {
  ...
  event_ = reactesp::EventLoop::app->onDelay(pattern_[pattern_ptr_++],
                                               [this]() { this->tick(); });
}
