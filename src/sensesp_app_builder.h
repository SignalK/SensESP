...
class SensESPAppBuilder : public SensESPBaseAppBuilder {
 ...
 public:
   ...
   * restarts the device. The timeout is exposed on the web UI Configuration
   * page as the "WiFi Watchdog" card; a value saved there takes precedence
   * over @p timeout_s on later boots. Call this before get_app().
   *
  ...
  SensESPAppBuilder* enable_wifi_watchdog(int timeout_s = 180) {
    ...
    if (app_->network_state_producer_) {
      ESP_LOGE(__FILENAME__,
               "enable_wifi_watchdog() must be called before get_app(); "
