...
**Note:** In SensESP v3, `AnalogInput` is deprecated. The modern alternative is to use a `RepeatSensor` with `analogReadMilliVolts()`. The approach described in this tutorial still works, but new projects should prefer `RepeatSensor`.
