...
When you read an analog input pin directly, the raw value is an integer whose range depends on the microcontroller and how its ADC is configured - on an ESP32 it is typically 0 - 4095. SensESP's [`AnalogInput` Sensor](https://signalk.org/SensESP/generated/docs/class_analog_input.html) hides that difference for you: it reads the pin as a calibrated voltage, normalizes that against the 3.3 V full-scale reference, and multiplies the result by a parameter called `output_scale`. The output is therefore always in the range 0 to `output_scale`, whichever ESP32 variant you are using. The default value for `output_scale` is 1024. Note that `output_scale` only chooses the units of the output - it does not affect the precision of the reading. Since 1024 is usually not a meaningful number to us humans - we like things expressed in percentages, or ratios, or sometimes flushes, as you'll see below - the use of the AnalogInput Sensor (and probably the Linear Transform) will be explained in hopefully simple terms, to help you get an appropriate number out of an AnalogIn pin and into Signal K.
...
tank_level->connect_to(new SKOutputFloat(sk_path)); // assumes sk_path has already been defined
```
